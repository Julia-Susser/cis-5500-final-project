...
const author = async function(req, res) {
  ...
  const name = 'Julia Susser';
  const pennkey = 'jsusser';
  ...
  `, (err, data) => {
    if (err) {
      ...
      res.json({
        ...
        title: data.rows[0].title,  // Added title to the response
      });
    }
  });
}
...
const song = async function(req, res) {
  ...
  connection.query(`SELECT * 
    FROM Songs 
    WHERE song_id = '${song_id}'`, (err, data) => {
    if (err) {
      console.log(err);
      res.json({});
    } else {
...
const albums = async function(req, res) {
  ...
  `, (err, data) => {
    if (err) {
      ...
      res.json(data.rows);  // Return an array of album objects
    }
  });
};
...
const top_songs = async function(req, res) {
  ...
  const pageSize = req.query.page_size ?? 10; // Use nullish coalescing to set default pageSize to 10
...
const search_songs = async function(req, res) {
  ...
  const explicit = req.query.explicit === 'true' ? 1 : 0;
  query = ` SELECT song_id, album_id, title, number, duration, plays, 
